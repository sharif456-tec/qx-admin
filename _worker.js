...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      return json({ ok:false, error:e?.message || 'Unexpected server error.' }, e?.status || 500);
    }
  }
};
...
function corsHeaders(request) {
  ...
  return { ...JSON_HEADERS, 'access-control-allow-origin': origin, 'access-control-allow-headers': 'content-type, authorization', 'access-control-allow-methods': 'POST, OPTIONS' };
}
...
async function register(request, env) {
  ...
  const telegramUsername = telegram.replace(/^@/, '').trim().toLowerCase();
  if (telegramUsername) {
    const tgLookup = await supabase(request, env, `/rest/v1/telegram_chats?select=chat_id,username&username=ilike.${encodeURIComponent(telegramUsername)}&limit=1`);
    if (tgLookup.ok) {
      const tgRows = await tgLookup.json();
      telegramChatId = tgRows?.[0]?.chat_id ? String(tgRows[0].chat_id) : null;
    }
  }
  ...
  const insertResp = await supabase(request, env, '/rest/v1/license_registrations', {
    ...
    body:JSON.stringify({name,email,telegram,telegram_chat_id:telegramChatId,device_id:deviceId,device_name:deviceName,status:'pending'})
  });
  if (!insertResp.ok) throw new Error(`Supabase registration save failed: ${await insertResp.text()}`);
  const rows = await insertResp.json();
  return new Response(JSON.stringify({ok:true,status:'pending',request_id:rows?.[0]?.id || null,telegram_linked:!!telegramChatId}), {headers:corsHeaders(request)});
}
...
async function resendLicense(request, env) {
  ...
  const lic = rows?.[0];
  if (!lic) throw httpError('License not found.', 404);
  const q = await supabase(request, env, `/rest/v1/license_registrations?select=*&license_id=eq.${encodeURIComponent(licenseId)}&limit=1`);
...
async function telegramSend(env, chatId, lic) {
  ...
  if (!chatId) return {ok:false,error:'Applicant has no Telegram Chat ID. Ask them to start the bot first.'};
  const text = [
    '✅ QX LICENSE APPROVED', '',
    `License: ${lic.license_key || ''}`,
    lic.plan ? `Plan: ${lic.plan}` : null,
    lic.expires_at ? `Expires: ${lic.expires_at}` : null,
    lic.max_devices ? `Device Limit: ${lic.max_devices}` : null,
    '', 'Your QX license has been activated. Enter this key in the extension.'
  ].filter(Boolean).join('\n');
...
async function telegramWebhook(request, env) {
  ...
  const save = await supabase(request, env, '/rest/v1/telegram_chats', {
    ...
    body:JSON.stringify({chat_id:chatId,username,first_name:firstName,last_name:lastName,start_payload:startArg || null,last_seen_at:new Date().toISOString()})
  });
  if (!save.ok) throw new Error(`Supabase telegram_chats write failed: ${await save.text()}`);

  if (startArg || username) {
    await supabase(request, env, '/rest/v1/rpc/link_telegram_chat', {
      method:'POST', body:JSON.stringify({p_email:startArg || '',p_telegram_chat_id:chatId,p_username:username})
    });

    if (username) {
      await supabase(request, env, `/rest/v1/license_registrations?telegram=ilike.${encodeURIComponent('@' + username)}&status=eq.pending`, {
        method:'PATCH', body:JSON.stringify({telegram_chat_id:chatId,updated_at:new Date().toISOString()})
      });
      await supabase(request, env, `/rest/v1/license_registrations?telegram=ilike.${encodeURIComponent(username)}&status=eq.pending`, {
        method:'PATCH', body:JSON.stringify({telegram_chat_id:chatId,updated_at:new Date().toISOString()})
      });
  ...
  const reply = '✅ Telegram connected. Your Chat ID has been registered. If you already submitted a license request, it is now linked to this Telegram account.';
