...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/telegram-set-webhook' && request.method === 'GET') return await telegramSetWebhook(env);
...
async function telegramSetWebhook(env) {
  if (!env.TELEGRAM_BOT_TOKEN) return json({ok:false,error:'TELEGRAM_BOT_TOKEN is not configured.'},500);
  const webhookUrl = 'https://qx-admin.pages.dev/api/telegram-webhook';
  const resp = await fetch(`https://api.telegram.org/bot${encodeURIComponent(env.TELEGRAM_BOT_TOKEN)}/setWebhook`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({url:webhookUrl, allowed_updates:['message'], drop_pending_updates:false}) });
  const data = await resp.json().catch(()=>({}));
  if (!resp.ok || !data.ok) return json({ok:false,error:data.description||'Telegram setWebhook failed.',telegram:data},502);
  return json({ok:true,webhook_url:webhookUrl,message:data.description||'Webhook was set'});
}
...
async function telegramStatus(env) {
  ...
  const meResp = await fetch(`https://api.telegram.org/bot${encodeURIComponent(env.TELEGRAM_BOT_TOKEN)}/getMe`); const me = await meResp.json().catch(() => ({}));
  ...
  const whResp = await fetch(`https://api.telegram.org/bot${encodeURIComponent(env.TELEGRAM_BOT_TOKEN)}/getWebhookInfo`); const wh = await whResp.json().catch(() => ({}));
  ...
  result.webhook_url = wh.result?.url || null; result.webhook_status = result.webhook_url === 'https://qx-admin.pages.dev/api/telegram-webhook' ? 'configured' : (result.webhook_url ? 'wrong_url' : 'not_configured'); result.last_update = wh.result?.last_error_date ? new Date(wh.result.last_error_date * 1000).toISOString() : null; result.telegram_webhook_last_error = wh.result?.last_error_message || null; result.pending_update_count = wh.result?.pending_update_count ?? null;
  try { const sb = await supabase(null, env, '/rest/v1/telegram_chats?select=chat_id,username,last_seen_at&order=last_seen_at.desc&limit=1'); if (sb.ok) { const rows=await sb.json(); result.supabase_status='connected'; if(rows?.[0]){result.last_chat_id=String(rows[0].chat_id);result.last_username=rows[0].username||null;result.last_update=rows[0].last_seen_at||result.last_update;} } else { result.supabase_status='error'; result.supabase_error=await sb.text(); } } catch(e){ result.supabase_status='error'; result.supabase_error=e?.message||String(e); }
  return json(result);
}
...
async function requireAdmin(request, env) {
  ...
  if (!token) throw httpError('Missing admin authentication.', 401);
  const userResp = await fetch(`${env.SUPABASE_URL}/auth/v1/user`, { headers: { apikey: env.SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${token}` } });
  if (!userResp.ok) { const detail = await userResp.text().catch(() => ''); throw httpError(`Supabase session is invalid or expired.${detail ? ` ${detail}` : ''}`, 401); }
...
async function register(request, env) {
  const body = await request.json(); const name=String(body.name||'').trim(), email=String(body.email||'').trim().toLowerCase(), telegram=String(body.telegram||'').trim(), deviceId=String(body.device_id||'').trim(), deviceName=String(body.device_name||'Kiwi/Chrome Android').trim();
  if(!name||!email||!telegram||!deviceId)return new Response(JSON.stringify({ok:false,error:'Name, email, Telegram and device ID are required.'}),{status:400,headers:corsHeaders(request)}); if(!/^\S+@\S+\.\S+$/.test(email))return new Response(JSON.stringify({ok:false,error:'Invalid email address.'}),{status:400,headers:corsHeaders(request)});
...
async function approveLicense(request, env) { await requireAdmin(request,env); const body=await request.json();const requestId=String(body.request_id||'').trim();if(!requestId)throw httpError('request_id is required.',400);const rpc=await supabase(request,env,'/rest/v1/rpc/admin_approve_license_registration',{method:'POST',body:JSON.stringify({p_registration_id:Number(requestId)})});const data=await rpc.json().catch(()=>({}));if(!rpc.ok||!data?.ok)return json({ok:false,error:data?.message||data?.hint||data?.error||'Supabase approval failed.'},400);const lic=data.license||{},req=data.registration||data.request||{};const sent=await telegramSend(env,String(req.telegram_chat_id||'').trim(),lic);if(!sent.ok)return json({ok:false,error:`License activated in Supabase, but Telegram delivery failed: ${sent.error}`,license:lic,delivery_failed:true},502);await supabase(request,env,`/rest/v1/licenses?id=eq.${encodeURIComponent(lic.id)}`,{method:'PATCH',body:JSON.stringify({updated_at:new Date().toISOString()})});return json({ok:true,license:lic,telegram:sent}); }
async function resendLicense(request, env) { await requireAdmin(request,env);const body=await request.json();const licenseId=String(body.license_id||'').trim();if(!licenseId)throw httpError('license_id is required.',400);const r=await supabase(request,env,`/rest/v1/licenses?select=*&id=eq.${encodeURIComponent(licenseId)}&limit=1`);if(!r.ok)throw new Error(await r.text());const rows=await r.json(),lic=rows?.[0];if(!lic)throw httpError('License not found.',404);const q=await supabase(request,env,`/rest/v1/license_registrations?select=*&license_id=eq.${encodeURIComponent(licenseId)}&limit=1`);if(!q.ok)throw new Error(await q.text());const req=(await q.json())?.[0];if(!req?.telegram_chat_id)throw httpError('No Telegram Chat ID is linked to this license.',400);const sent=await telegramSend(env,req.telegram_chat_id,lic);if(!sent.ok)return json({ok:false,error:sent.error},502);await supabase(request,env,`/rest/v1/licenses?id=eq.${encodeURIComponent(licenseId)}`,{method:'PATCH',body:JSON.stringify({updated_at:new Date().toISOString()})});return json({ok:true,telegram:sent}); }
async function sendLicense(request, env) { await requireAdmin(request,env);const body=await request.json();const chatId=String(body.telegram_chat_id||'').trim(),licenseKey=String(body.license_key||'').trim();if(!chatId||!licenseKey)throw httpError('telegram_chat_id and license_key are required.',400);return json(await telegramSend(env,chatId,{license_key:licenseKey,plan:body.plan,expires_at:body.expires_at,max_devices:body.max_devices})); }
