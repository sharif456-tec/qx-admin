...
async function requireAdmin(request, env) {
  ...
  // Validate the user's access token through Supabase Auth. Use the server key for
  // the API key header; the user's Bearer token remains the credential being checked.
  ...
  if (!userResp.ok) {
    ...
    throw httpError(`Supabase session is invalid or expired.${detail ? ` ${detail}` : ''}`, 401);
  }
