...
async function approveLicense(request, env) { await requireAdmin(request,env); const body=await request.json();const requestId=String(body.request_id||'').trim();if(!requestId)throw httpError('request_id is required.',400);const rpc=await supabase(request,env,'/rest/v1/rpc/admin_approve_license',{method:'POST',body:JSON.stringify({p_request_id:requestId})});const data=await rpc.json().catch(()=>({}));if(!rpc.ok||!data?.ok)return json({ok:false,error:data?.message||data?.hint||data?.error||'Supabase approval failed.'},400);const lic=data.license||{},req=data.registration||data.request||{};const sent=await telegramSend(env,String(req.telegram_chat_id||'').trim(),lic);if(!sent.ok)return json({ok:false,error:`License activated in Supabase, but Telegram delivery failed: ${sent.error}`,license:lic,delivery_failed:true},502);await supabase(request,env,`/rest/v1/licenses?id=eq.${encodeURIComponent(lic.id)}`,{method:'PATCH',body:JSON.stringify({last_telegram_sent_at:new Date().toISOString(),updated_at:new Date().toISOString()})});return json({ok:true,license:lic,telegram:sent}); }
