...
async function register(request, env) {
  ...
  const existingResp = await supabase(request, env, `/rest/v1/license_requests?select=id,status&email=eq.${encodeURIComponent(email)}&device_id=eq.${encodeURIComponent(deviceId)}&status=eq.pending&limit=1`);
  ...
  const insertResp = await supabase(request, env, '/rest/v1/license_requests', {
    method:'POST',
    headers:{Prefer:'return=representation'},
    body:JSON.stringify({name,email,telegram,device_id:deviceId,device_name:deviceName,status:'pending'})
  });
...
async function resendLicense(request, env) {
  ...
  const q = await supabase(request, env, `/rest/v1/license_requests?select=*&license_id=eq.${encodeURIComponent(licenseId)}&limit=1`);
